...
def criterio_real_name(personaje):
    if personaje["real_name"]:
        return personaje["real_name"]
    else:
        return ""

def criterio_first_appearance(personaje):
    return personaje["first_appearance"]

# 1.
superheroes.add_criterion("name", criterio_nombre)
...
pos_the_thing = buscar_posicion("The Thing")
pos_rocket = buscar_posicion("Rocket Raccoon")
print("2. Posición de The Thing:", pos_the_thing)
print("2. Posición de Rocket Raccoon:", pos_rocket)
print()

# 3.
villanos = List()
for i in range(superheroes.size()):
    if superheroes[i]["is_villain"]:
        villanos.add(superheroes[i])

# 4
cola_villanos = Queue()
for i in range(villanos.size()):
    cola_villanos.arrive(villanos[i])

cola_antes_1980 = Queue()
for i in range(cola_villanos.size()):
    villano = cola_villanos.at(i)
    if villano["first_appearance"] < 1980:
        cola_antes_1980.arrive(villano)

# 5.
iniciales = ("Bl", "G", "My", "W")
personajes_filtrados = List()
for i in range(superheroes.size()):
    nombre = superheroes[i]["name"]
    if not superheroes[i]["is_villain"]:
        for prefijo in iniciales:
            if nombre.startswith(prefijo):
                personajes_filtrados.add(superheroes[i])
                break

# 6.
superheroes.add_criterion("real_name", criterio_real_name)
superheroes.sort_by_criterion("real_name")

# 7.
superheroes.add_criterion("first_appearance", criterio_first_appearance)
superheroes.sort_by_criterion("first_appearance")

# 8.
for i in range(superheroes.size()):
    if superheroes[i]["name"] == "Ant Man":
        superheroes[i]["real_name"] = "Scott Lang"

# 9.
print("9. Personajes con 'time-traveling' o 'suit' en su biografía:")
for i in range(superheroes.size()):
    bio = superheroes[i].get("short_bio", "").lower()
    if "time-traveling" in bio or "suit" in bio:
        print(superheroes[i]["name"], "-", superheroes[i]["short_bio"])
print()
...
for nombre in ["Electro", "Baron Zemo"]:
    ...
    for i in range(superheroes.size()):
        if superheroes[i]["name"] == nombre:
            ...
            break
    if pos is not None:
        eliminados.append(superheroes.pop(pos))
print("10. Eliminados Electro y Baron Zemo (si estaban):")
